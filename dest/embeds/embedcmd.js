        ...
        client.on("interactionCreate", async (i) => {
            ...
            if (i instanceof CommandInteraction && i.commandName == "embed") {
                ...
                if (group == "send") {
                    ...
                    if (!authorized)
                        return i.reply({ content: `Only server admins / mods may use this command.`, ephemeral: true });
                    if (subcmd == "new") {
                    ...
                    if (subcmd == "add") {
                        ...
                        if (!fields[index])
                            return i.reply({ content: `There is no such field with index ${index} in ${id}.\nTry /embed view or /embed add`, ephemeral: true });
                    ...
                    let userEmbeds = embeds.get(i.user.id) || new MagicMap();
                    if (embeds.has(id))
                        return i.reply({ content: `An embed with the id ${id} already exists.\nTry /embed edit or /embed list`, ephemeral: true });
                    i.reply({ content: o.content, embeds: [o.embed], ephemeral: true })
                        .then(() => {
                        let userEmbeds = embeds.get(i.user.id) || new MagicMap();
                        userEmbeds.set(id, o);
                    ...
                    if (ids)
                        i.reply({ content: `Embeds: ${ids.join(", ")}`, ephemeral: true });
                    else if (ids)
                        i.reply({ content: "You did not create any embeds yet.\nTry /embed create", ephemeral: true });
                    return;
                }
                else if (subcmd == "edit") {
                    let id = i.options.getString("id") || "";
                    ...
                    if (!userEmbeds.has(id))
                        return i.reply({ content: `You did not create an embed with id ${id}.\nTry /embed list or /embed create`, ephemeral: true });
                ...
                if (i.commandName == "embed") {
                    ...
                    if (opt.name == "id") {
                        let userEmbeds = embeds.get(i.user.id) || new MagicMap();
                        i.respond(autocomplete(userEmbeds, (k, v) => k.includes(opt.value), (k, v) => ({ name: `${k} (${v.embed.title || v.embed.description || console.log(v)})`, value: k })));
                    }
                }
            }
        });
