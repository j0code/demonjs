...
export function load(path: string, ignore: boolean = false) {
	return fs.readFile(savePath + path + ".json", {encoding: "utf8"})
	.then(data => JSON.parse(data))
	.catch(e => {
		if(e.code == "ENOENT" || ignore) return
		console.error(`Error loading save ${"./saves/" + path}:`, e)
		process.kill(0)
	})
}

export function write(path: string, content: any) {
	if(!content) return console.error(`Error: save.write(${path}): content empty!`)
...
export function obj(map: Map<any, any>) {
	if(!map) return {}
