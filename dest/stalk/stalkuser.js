...
export default class StalkUser extends EventEmitter {
    ...
    get status() {
        if (this.#status == "offline" && this.active && !this.#justLeft)
            return "invisible";
        return this.#status;
    }
    get deviceStatus() {
    ...
    set presence(presence) {
        ...
        if (presence.status != this.#status && presence.status == "offline")
            this.#justLeft = true;
        if (["offline", "idle", "dnd", "online"].includes(presence.status))
        ...
        if (sB != this.status || !compare(dSB, this.#deviceStatus))
