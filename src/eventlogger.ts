...
import ClientEventData from "./util/clienteventdata.js"
...
export default function logEvents(client: Client, logApiEvents = false, logShardEvents = false, debug = false) {
...
function log(e: string, ...content: any[]) {
...
function getEventOptions(name: string) {
	name = camelToUpper(name || "")
	if(events.SHARD.includes(name)) return { name, emoji: "✨", color: "93" }
	if("CHANNEL_PINS_UPDATE" == name) return { name, emoji: "📌" }
	if("ERROR" == name) return { name, emoji: "🔴", color: "41;97" }
	if("GUILD_CREATE" == name) return { name, emoji: "➡️ ", color: "32" }
	if("GUILD_DELETE" == name) return { name, emoji: "⬅️ ", color: "91" }
	if("INTERACTION_CREATE" == name) return { name, emoji: "🔥", color: "95"  }
	if(["INVALID_REQUEST_WARNING","RATE_LIMIT","WARN"].includes(name)) return { name, emoji: "⚠️ ", color: "43;90"  } // 🟡
	if(["MESSAGE_CREATE","MESSAGE_UPDATE"].includes(name)) return { name, emoji: "✍️ ", color: "34" } // 💬📝
	if("READY" == name) return { name, emoji: "✅", color: "92"  }
	if("THREAD_LIST_SYNC" == name) return { name, emoji: "🔁" }
	if("TYPING_START" == name) return { name, emoji: "💬" }
	if("VOICE_STATE_UPDATE" == name) return { name, emoji: "🎤" } // 🎙️
	if("WEBHOOK_UPDATE" == name) return { name, emoji: "⚓" }
	if(name.includes("GUILD_BAN")) return { name, emoji: "🚫" }
	if(name.includes("THREAD")) return { name, emoji: "🧵" }
	if(name.includes("ADD")) return { name, emoji: "➕", color: "32"  }
	if(name.includes("REMOVE")) return { name, emoji: "➖", color: "91"  }
	if(name.includes("CREATE")) return { name, emoji: "🟢", color: "32" }
	if(name.includes("UPDATE")) return { name, emoji: "🔄", color: "94"  }
	if(name.includes("DELETE")) return { name, emoji: "❌", color: "91"  }
	return { name, emoji: "  " }
}
