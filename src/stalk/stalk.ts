...
var users: MagicMap<StalkUser>

type EventData = Emoji | GuildMember | GuildScheduledEvent | Interaction | Invite | Message | Sticker | Typing | User | Presence | VoiceState

export default class Stalk extends EventEmitter {

	#activeUsers: Set<string> // needed to allow them to emit 'inactive'
	...
	onUserEvent(ename: string, user: User, a: EventData, b: EventData) {
		if(a && "webhookId" in a) return // ignore webhuks
		...
		if (a instanceof Presence && b instanceof Presence) {
			if (b.status != "offline" && b.status != a.status) stalkUser.seen() // don't count "going offline" as witness
