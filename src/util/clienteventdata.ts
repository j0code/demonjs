...
export default class ClientEventData {
	...
	e; a; b; c; request; response; time; info; ban; members; chunk; scheduledEvent;
	interaction; invalidRequestWarningData; invite; messages; reactions; presence;
	rateLimitData; shardId; sticker; thread; threads; typing; voiceState; before; now; 

	constructor(e: string, a: any, b: any, c: any) {
		this.e = e
		if(a) this.a = a
		if(b) this.b = b
		if(c) this.c = c
		...
		if(events.API.includes(e)) {
			...
			this.#emoji = a || b
		} else if(events.GUILD_BAN.includes(e)) this.ban = a
			...
			if(events.GUILD_SCHEDULED_EVENT_USER.includes(e)) this.#user = b
