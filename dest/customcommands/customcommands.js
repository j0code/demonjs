    ...
    constructor() {
        client.on("interactionCreate", i => {
            ...
            if (i.commandName == "command") {
                if (i.options.getSubcommand() == "set") {
                    ...
                    if (delmsg)
                        reply = false; // can't reply to deleted message
                    if (!content && !reaction) {
                        i.reply({ ephemeral: true, content: "You have to specify at least one of: content, reaction" });
                        return;
                    }
                    if (!content && reaction && delmsg) {
                        i.reply({ ephemeral: true, content: "delmsg can not be used together with reaction-only command" });
                        return;
                    }
                    console.log(`command set: "${name}", "${content}", ${reply}, "${reaction}, ${delmsg}`);
                    let cmd = { content, reply, reaction, delmsg };
                    ...
                    for (let name of commands.keys())
                        names.push(name);
                    if (names.length > 0)
                        i.reply({ ephemeral: true, content: "```js\n" + names.join(", ") + "```" });
                    else
                        i.reply({ ephemeral: true, content: "https://cdn.discordapp.com/attachments/580366338133065738/914289479060181042/empathy_banana.png" });
                }
                    ...
                    if (commands.has(name)) {
                        ...
                        i.reply({ ephemeral: true, content: "Unknown command." });
                }
                    ...
                    let name = i.options.getString("name") || "";
                    let content = i.options.getString("content") || "";
                    let reply = i.options.getBoolean("reply");
                    let reaction = i.options.getString("reaction") || "";
                    ...
                    let cmd = commands.get(name);
                    if (!cmd) {
                        i.reply({ ephemeral: true, content: "Unknown command. Try `/command set` instead" });
                        return;
                    }
                    if (!content && !cmd.content && (reaction || cmd.reaction) && delmsg) {
                        i.reply({ ephemeral: true, content: "delmsg can not be used together with reaction-only command" });
                        return;
                    }
        ...
        client.on("messageCreate", msg => {
            ...
            if (cmd.content) {
                let react = (m) => {
                    if (cmd?.reaction)
                        m.react(substDollar(cmd.reaction, name, args, msg, true))
                            .catch(e => msg.reply("Invalid reaction: " + cmd?.reaction));
                };
                if (cmd.reply)
                    msg.reply(substDollar(cmd.content, name, args, msg, false)).then(react);
                else
                    msg.channel.send(substDollar(cmd.content, name, args, msg, false)).then(react);
                if (cmd.delmsg)
                    msg.delete()
                        .catch(e => {
...
function substDollar(r, name, args, msg, nospecial) {
    ...
    for (let i = 0; i < a.length; i++) {
        ...
        else if (!nospecial && a[i] == "u") {
            v = msg.author.tag;
            n = "u";
            i++;
        }
        else if (!nospecial && a[i] == "m") {
            v = "<@" + msg.author.id + ">";
            n = "m";
            i++;
        }
        else if (!nospecial && a[i] == "d") {
            v = getDateString();
            n = "d";
            i++;
        }
        else if (!nospecial && a[i] == "t") {
            v = getTimeString();
            n = "t";
            i++;
        }
        else if (!nospecial && a[i] == "n") {
            v = "\n";
            n = "n";
            i++;
        }
        else {
            while (!isNaN(Number(a[i])) && a[i].trim() != "") {
                console.log(a[i], n, i);
                n += a[i];
                i++;
            }
