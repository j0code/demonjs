...
config.clientOptions.sweepers = {
    guildMembers: { interval: 11, filter: () => (v) => {
            ...
            if (sweep && process.argv[2] == "debug")
                console.log(`[SWEEP] Sweeping member ${v.user.tag} of ${v.guild.name}!`);
            return sweep;
        } },
    messages: { interval: 11, lifetime: 60 * 1000 },
    users: { interval: 11, filter: () => (v) => {
            ...
            let sweep = (Date.now() - stalkUser.lastSeen) > 60 * 1000; // last seen 1 min ago
            if (sweep && process.argv[2] == "debug")
                console.log(`[SWEEP] Sweeping user ${v.tag}!`);
            return sweep;
        } }
};
client.on("ready", () => {
...
client.on("messageCreate", (msg) => {
    if (msg.content.startsWith("stalk")) {
        ...
        if (isNaN(Number(id)))
            id = id.substring(2, id.length - 1); // strip <@ >
        if (isNaN(Number(id)))
            id = id.substring(1); // string ! of <@! >
        if (isNaN(Number(id)))
            return msg.reply("Syntax: `stalk id|@mention`");
        var user = client.users.cache.get(id);
        if (!user)
            return msg.reply("Unknown user.");
        var stalkUser = stalk.getUser(id);
        if (!stalkUser)
            return msg.reply(`**${user.tag} Stalk Info**\nStatus: unknown\nActive: false\nLast seen: Never`);
