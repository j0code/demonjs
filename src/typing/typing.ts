...
type TypingChannels = Map<string, TypingData>

const pocTime = 45000 // 45s
const typingTime = 11000 // 11s
const typingUsers: Map<string, TypingChannels> = new Map()
...
export function listen() {
	...
	client.on("messageCreate", msg => {
		...
		if (!channels || !channels.has(msg.channel.id)) return
		...
		if (o.msg) {
			o.msg.delete()
			.catch((e: any) => console.error("[Typing/ERROR]: Could not delete poc msg:", e))
		}
	})
	...
	for(let uid of typingUsers.keys()) {
		...
		for(let cid of channels.keys()) {
			...
			if(now - o.last > typingTime) {
				...
				o.pocs++
				if (!o.msg) {
					c.send("póc").then((m: Message) => { if (!o) return; o.msg = m })
					.catch((e: any) => console.error("[Typing/ERROR]: Could not send poc msg:", e))
				} else {
					o.msg.edit(`póc (${o.pocs})`).then((m: Message) => { if (!o) return; o.msg = m })
					.catch((e: any) => console.error("[Typing/ERROR]: Could not edit poc msg:", e))
				}
			}
		}
	}
...
function getTypingChannels(id: string): TypingChannels | undefined {
	let channels: TypingChannels | undefined
...
function getTypingChannel(channels: TypingChannels | undefined, id: string): TypingData | undefined {
	//console.log(channels)
	if (!channels) return
	let now = Date.now()
	let o: TypingData | undefined
	if (!channels.has(id)) {
		//console.log("no")
