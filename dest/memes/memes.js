...
function onCommand(i) {
    ...
    if (!template)
        return i.reply({ content: "Template not found.", ephemeral: true });
    ...
    if (template.texts?.length >= 1)
        drawText(canvas, ctx, template, template.texts[0], text);
    if (template.texts?.length >= 2)
        drawText(canvas, ctx, template, template.texts[1], text2);
    if (template.texts?.length >= 3)
        drawText(canvas, ctx, template, template.texts[2], text3);
    if (template.texts?.length >= 4)
        drawText(canvas, ctx, template, template.texts[3], text4);
    if (template.names?.length >= 1)
        drawText(canvas, ctx, template, template.names[0], name);
    if (template.names?.length >= 2)
        drawText(canvas, ctx, template, template.names[1], name2);
    if (template.names?.length >= 3)
...
function onComplete(i) {
    ...
    for (let index in templates) {
        ...
        if (name == tname)
            return i.respond([{ name: t.name, value: Number(index) }]);
        if (tname.startsWith(name))
            matchingStarts.push({ name: t.name, value: Number(index) });
        else if (tname.includes(name))
            matchingIncludes.push({ name: t.name, value: Number(index) });
    }
