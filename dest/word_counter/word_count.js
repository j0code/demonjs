        ...
        client.on("interactionCreate", (i) => {
            if (i instanceof ChatInputCommandInteraction) {
                if (i.commandName == "wordcount") {
                    ...
                    if (word) {
                        if (user) {
                            ...
                            for (let e of users) {
                                ...
                                if (special_ids.webhooks.has(id))
                                    name = special_ids.webhooks.get(id);
                        ...
                        arr = Array.from(Object.entries(o));
                    ...
                    if (!s)
                        i.reply({ content: "Error: empty list", ephemeral: false, fetchReply: true })
                            .then(msg => msg.react("❌"));
                    else
                        i.reply({ content: `**TOTAL: ${totalcount}**${(user || word) ? ` *(${percent(totalcount, total)}% of global)*` : ""}\n${s.join("\n")}`, ephemeral: false, fetchReply: true })
                            .then(msg => msg.react("❌"));
                }
                ...
                if (i.commandName == "wordcount") {
                    ...
                    if (opt.name == "language") {
                        i.respond(autocomplete(languages, (k, v) => (v.name.includes(opt.value) || v.name_en.includes(opt.value) || v.code.startsWith(opt.value)), (k, v) => ({ name: `${v.name} (${v.name_en})`, value: v.code })));
                    }
...
function dictionaryCmd(i) {
    ...
    if (subcmd == "show")
        showDict(i, "DICTIONARY", words);
    else if (subcmd == "langs")
        showLangs(i, "LANGUAGES", languages);
    else if (subcmd == "addlang")
        addLang(i);
    else
        i.reply("Oops. You got me. I don't know what to do.");
}
...
function addLang(i) {
    if (i.user.id != special_ids.owner)
        return i.reply({ content: "This command is for the bot owner only.", ephemeral: true });
    ...
    if (!code || !name || !name_en)
        return i.reply({ content: "Incomplete arguments", ephemeral: true });
    if (languages.has(code))
        return i.reply({ content: "This language already exists.", ephemeral: true });
    if (code.length != 2)
        return i.reply({ content: "The lang code has to be ISO 639-1", ephemeral: true });
